<?php 
class Controler {
	public $screenData = "<div style='display: grid;height: calc(100% - 50px);'>
		<h1 style='margin:auto'>Acccess Denied</h1>
		<a style='margin:auto' href='{{link}}'>{{name}}</a>
	</div>";
	...
	function __construct($glb) {
		...
		if(isset($_REQUEST["screen"])){
			...
			$this->screenData	=	str_replace("{{name}}","Login",$this->screenData);
			$this->screenData	=	str_replace("{{link}}",screen_url($this->config["invalid_page"]),$this->screenData);
